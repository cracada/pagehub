...
function ctor_structure()
{
  ...
  self.build = function() { // Write HTML before DOM is loaded to prevent flickering.
    ...
    if (!isFrameCapable)
      template = template.replace('<iframe frameBorder="0" id="frame" src="" role="main">', '<div class="area" role="main">');
    if (isIE)
